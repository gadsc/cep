...
public class CepClient {
	...
	public Endereco buscarEnderecoPorCep(String cep) throws CepServiceException {
		...
		try {
			...
			if (Status.BAD_REQUEST.equals(response.getStatus())) {

				jsonEndereco = restClient.getJsonFromResponse(response);
				...
				endereco.setCidade(ObjectUtils.toString(jsonEndereco
						.get(JsonEnderecoKeys.CIDADE.toString())
						.getAsJsonObject()
				...
				endereco.setEstado(ObjectUtils.toString(jsonEndereco
						.get(JsonEnderecoKeys.CIDADE.toString())
						.getAsJsonObject()
						.get(JsonEnderecoKeys.ESTADO.toString())
						.getAsJsonObject()
						.get(JsonEnderecoKeys.NOME.toString())));
				...
				throw new CepServiceException(response.getEntity().toString());
			}
		} catch (Exception err) {
			err.printStackTrace();
		}
